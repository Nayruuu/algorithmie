...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/03/24 18:22:38 by sde-toda          #+#    #+#             */
/*   Updated: 2015/03/25 11:56:58 by sde-toda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_function(t_env poly, void *mlx, void *win)
{
	double		i;
	double		x;
	double		x2;
	...
	while (++i < 1361)
	{
		x = 30 * i / 1360 - 15;
		x2 = 30 * i / 1360 - 15;
		y2 = (poly.coeff[2] + (2 * poly.coeff[3] * x2));
		y2 = 1000 - (y2 + 11) * 1000 / 22;
		y = (poly.coeff[0] + poly.coeff[1]) + (x * poly.coeff[2]) +
			(x * x * poly.coeff[3]);
		y = 1000 - (y + 11) * 1000 / 22;
...
static int		gere_key(int keycode, t_env *poly)
{
	*poly = (t_env)*poly;
	if (keycode == 53)
		exit (1);
...
void			do_mlx(t_env poly)
{
	...
	draw_graph(mlx, win);
	draw_tirets(mlx, win);
	draw_function(poly, mlx, win);
	mlx_key_hook(win, gere_key, (void*)&poly);
	mlx_loop(mlx);
}
